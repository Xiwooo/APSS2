...
$stmt = $db->prepare(
    "SELECT ID_STAGE,DATE_FIN,DATE_DEBUT, NOM_ETUDIANT,
      ...
      WHERE SALARIE.ID_SALARIE=STAGE.ID_SALARIE AND 
           ...
           SPECIALITE.ID_PROF=:id    AND ETAT NOT IN('OK','RE') 
...
$stmt = $db->prepare(
    "SELECT  demarche.ID_ETUDIANT,NOM_ETUDIANT,PRENOM_ETUDIANT,COUNT(ID_DEMARCHE)  AS NB_DEM  
...
$stmt = $db->prepare(
    "SELECT demarche.ID-ETUDIANT,NOM_ETUDIANT,PRENOM_ETUDIANT,COUNT(ID_DEMARCHE)  AS NB_DEM
      FROM etudiant
